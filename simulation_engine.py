...
def run_simulation(theme_name_or_user_story: str = None,
    ...
    if is_user_story and theme_name_or_user_story:
        if VERBOSE_LOGGING: print(f"\nAttempting to generate custom theme from user story...")
        ...
        if custom_theme_dict:
            ...
            if VERBOSE_LOGGING: print(
                f"Successfully generated custom theme: {current_run_theme_config.get('description', 'N/A')}")
        else:  # LLM theme generation failed
            print("Failed to generate theme from user story. Falling back to a random predefined theme.")
            ...
            if current_run_theme_config and "description" not in current_run_theme_config:  # Error theme from get_random_theme
                print(
                    f"CRITICAL FALLBACK ERROR: Could not load a random theme ('{theme_name_or_user_story}'). Aborting.");
                return None, None
            print(f"Using fallback predefined theme: {theme_name_or_user_story}")
    ...
    if not is_user_story:  # Handles predefined themes OR if custom story generation failed and fell back
        ...
        if not chosen_theme_key:  # If None was passed and not a story
            ...
            if not current_run_theme_config:  # If specific theme not found by key
                print(f"Theme '{chosen_theme_key}' not found. Picking a random predefined theme.")
                chosen_theme_key, current_run_theme_config = get_random_theme()
                if current_run_theme_config and "description" not in current_run_theme_config:  # Error theme
                    print(
                        f"CRITICAL FALLBACK ERROR: Could not load random theme after specific fail ('{chosen_theme_key}'). Aborting.");
                    return None, None

        if current_run_theme_config and "description" not in current_run_theme_config:  # Final check after all attempts
            print(
                f"CRITICAL ERROR: Theme '{chosen_theme_key}' seems to be an error placeholder from theme_manager. Aborting.");
            return None, None
    ...
    if not current_run_theme_config:  # If still no theme after all logic
        print("FATAL: Could not load or generate any theme configuration. Aborting simulation.")
        return None, None
    # --- End Theme Loading ---
    ...
    print(
        f"\n--- Starting Simulation: House of {family.dynasty_name} ({current_run_theme_config.get('description')}) ---")
    print(
        f"Years: {effective_start_year} to {effective_start_year + effective_sim_years - 1}, Rule: {family.succession_rule}, Initial Wealth: {family.dynasty_wealth}")
    ...
    for year_idx_offset in range(effective_sim_years):
        ...
        if VERBOSE_LOGGING and year_idx_offset > 0 and year_idx_offset % 25 == 0:  # Periodic major year marker
            ...
            print(
                f"\n--- Year: {current_simulation_year} | Dynasty: {family.dynasty_name} | Wealth: {family.dynasty_wealth} | Pop: {current_pop_for_log} ---")
        ...
        if VERBOSE_LOGGING and (year_idx_offset % 10 == 0 or year_idx_offset == eff_sim_years - 1):
            print(
                f"End of Year {current_simulation_year}: Living Population = {current_year_end_population}, Dynasty Wealth = {family.dynasty_wealth}")
        ...
        if family.current_monarch is None and year_idx_offset > 7:  # Grace period of ~7 years after losing monarch
            ...
            if current_living_nobles_count == 0:
                print(f"\n--- SIMULATION END (Extinction of Nobles) ---")
                print(
                    f"Year {current_simulation_year}: No living leader and no living nobles remaining. Dynasty {family.dynasty_name} has effectively ended.")
                break  # Exit the main simulation loop

    # --- 5. End of Simulation Wrap-up ---
    _simulation_wrap_up(family, history_log, effective_start_year, eff_sim_years, dynasty_id_for_filenames)
